class DetermineChangeService
  ...
  def call
    if coin_in_best?
      current_price_in_euro = @best_bet_outcome.select {|e| e[0] == @coin_session.coin.symbol }[0][1]["data"]["price_eur"].to_f
      ...
      if current_value_of_crypto_in_euro >= @coin_session.last_known_value
        ...
        rest_amount.amount += current_value_of_crypto_in_euro - ENV["INITIAL_VALUE"]
        rest_amount.save
      else
        # I dunno
      end
